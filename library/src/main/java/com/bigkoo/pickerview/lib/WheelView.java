...
public class WheelView extends View {
    ...
    // 条目间距倍数
    static final float lineSpacingMultiplier = 1.6F;//1.4F
    boolean isLoop;
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        if (isLowDpiScreen(context)) {
            textSize = getResources().getDimensionPixelSize(R.dimen.pickerview_textsize_lowdpi);
            customTextSize = getResources().getBoolean(R.bool.pickerview_customTextSize_low_dpi);
        } else {
        ...
        if(attrs != null) {
            ...
            mGravity = a.getInt(R.styleable.wheelview_gravityW, Gravity.CENTER);
    ...
    private boolean isLowDpiScreen(Context context) {
        DisplayMetrics displayMetrics = context.getResources().getDisplayMetrics();
        int densityDpi = displayMetrics.densityDpi;
        if (densityDpi <= 240 && displayMetrics.heightPixels <= 800 && displayMetrics.widthPixels <= 480)
            return true;
        return false;
    }
